...
void Run::CountNuclearChannel(G4String name, G4double Q, G4double Emax)
{
  std::map<G4String, NuclChannel>::iterator it = fNuclChannelMap.find(name);
  if ( it == fNuclChannelMap.end()) {
    fNuclChannelMap[name] = NuclChannel(1, Q, Emax);
  }
  else {
  ...
  for (itc = localRun->fNuclChannelMap.begin(); 
    ...
    if ( fNuclChannelMap.find(name) == fNuclChannelMap.end()) {
      fNuclChannelMap[name]
       = NuclChannel(localData.fCount, localData.fQ, localData.fEmax);
    }
    else {
...
////// NOW USE THIS EMAX (MAYBE PRINT ONLY THOSE EVENTS CLOSE TO 1) TO ID PROCESSES WHIHC ARE 'BAD'
